...
(function(){
  ...
  function renderRad(r){
    ...
    if(r.typ === 'brak'){
      ...
      CHECKS.push(function(el){ return { ok: likhetOk(AK8_UI.cellRead(el, 'sv').num, r.ft / r.fn), facit: (r.ft < 0 ? '−' : '') + Math.abs(r.ft) + '/' + r.fn }; });
      return '<div class="ak8-rad"><span class="ak8-q">' + r.fraga + '</span><span class="ak8-svar" data-idx="' + idx + '">' + AK8_UI.ansCell('sv') + '</span></div>';
    }
    if(r.typ === 'kedja'){
    ...
    if(r.typ === 'skrivmult'){
      ...
      CHECKS.push(function(el){
        var s = AK8_UI.exprSerialize(el.querySelector('.ak8-cell[data-r="sv"] .ak8-expr')).replace(/[x×*]/g, '·').replace(/[()]/g, '');
  ...
  function renderBlad(mount, blad){
    ...
    AK8_UI.bindSheet(mount);   // keypad + grow + fokus + enter + clear-on-edit + skriv-ut
  }
