...
console.log('räknesätt   nivå  uppg   facit  band  dubbl  fördeln   drag/uppg  avslag (band | struktur)   [fördeln = nivå 3/4: alla profiler i varje omgång; stegvis-krav räknas i band]');
RAKNE.forEach(rakne => {
  ...
  Object.keys(band.nivaer).map(Number).forEach(niva => {   // alla nivåer i bandet (fyra där decimaltal finns)
    ...
    let uppg = 0, facitFel = 0, bandFel = 0, dubbl = 0, fordelnFel = 0, ex = [];
    for (let o = 0; o < OMG; o++) {
    ...
    fel += facitFel + bandFel + dubbl + fordelnFel;
    console.log(rakne.padEnd(11) + ' ' + String(niva).padEnd(5) + String(uppg).padEnd(6) + ' ' + String(facitFel).padEnd(6) + ' ' + String(bandFel).padEnd(5) + ' ' + String(dubbl).padEnd(6) + ' ' + String(fordelnFel).padEnd(9) + ' ' + drag.padEnd(10) + ' ' + avsl);
